...
7. **Observation 3:** Wait for processing to complete. Once finished, the button should re-enable and say "Update Preview" again. The main graphic panel must show the processed SVG.
8. Click **"Save As..."**, pick a destination file (e.g. `test_output.svg`), and verify the exact same asynchronous behavior happens (button disables, UI remains responsive) until you get the "Saved to..." success dialog.
